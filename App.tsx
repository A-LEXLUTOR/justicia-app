...
import DocumentCreationChoice from './components/DocumentCreationChoice';
...
const App: React.FC = () => {
    ...
    const [showDocumentCreationChoice, setShowDocumentCreationChoice] = useState(false);
    ...
    useEffect(() => {
        ...
        const foldersKey = `flagrFolders_${currentUser.id}`;
...
**Posez-moi n'importe quelle question sur ce document !**`,
    ...
    const handleSaveCollaborativeWork = useCallback((content: string, annotations: any[]) => {
        const document = {
            id: uuidv4(),
            title: editorTitle,
            content,
        ...
        return (
            <InitialView
                ...
                onAnalysisComplete={(analysisResult: AnalysisResult, fileName: string) => {
                    ...
                    // Forcer l'affichage de la vue d'analyse
                    // setCurrentView('analysis'); // Commenté pour le build
                    console.log('[App] Vue changée vers analysis');
    ...
    return (
        ...
        >
            ...
            {showDocumentCreationChoice && (
                <DocumentCreationChoice
                    onClose={() => setShowDocumentCreationChoice(false)}
                    onSelectFromTemplate={() => {
                        setShowDocumentCreationChoice(false);
                        setShowTemplateModal(true);
                    }}
                    onCreateFromScratch={() => {
                        setShowDocumentCreationChoice(false);
                        // Ouvrir l'éditeur collaboratif avec un document vierge
                        setShowCollaborativeEditor(true);
                    }}
                />
            )}
                        ...
                        <div className="flex-1 overflow-hidden">
                            <TemplateGallery 
                                ...
                                onSelectTemplate={(template) => {
                                    // Vérifier si le template contient du contenu pré-rempli
                                    if (template.content) {
                                        // Utiliser le contenu pré-rempli avec les valeurs du formulaire
                                        const prompt = `Voici un document pré-rempli avec les informations de PORTEO BTP. Formate-le de manière professionnelle en conservant toutes les informations et en améliorant la présentation si nécessaire :\n\n${template.content}`;
                                        handleSendMessage(prompt);
                                    } else {
                                        // Fallback : générer un document générique
                ...
                isSidebarExpanded ? (
                        ...
                        onShowVoiceChat={() => {
                            ...
                            if (activeSession?.analysis) {
                                ...
                                if (latestAnalysis.riskAssessment) {
                                    context += `⚠️ ÉVALUATION DES RISQUES:\n`;
                    ...
                    activeSession && (
                        <ChatPanel
                            ...
                            onShowTemplates={() => setShowDocumentCreationChoice(true)}
