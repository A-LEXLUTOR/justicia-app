...
interface CollaborativeEditorProps {
  ...
  onSave: (content: string, annotations: any[]) => void;
  onClose: () => void;
  title?: string;
}
...
const CollaborativeEditor: React.FC<CollaborativeEditorProps> = ({
  ...
  const exportAsImage = () => {
    ...
    link.download = `${title.replace(/\s+/g, '_')}_annotated.png`;
          ...
          <div className="max-w-4xl mx-auto">
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                // Exemple de modification basique - à améliorer avec vraie IA
                let modifiedContent = content;
                
                if (aiPrompt.toLowerCase().includes('corriger') || aiPrompt.toLowerCase().includes('corrige')) {
                  // Correction basique de ponctuation
                ...
                if (aiPrompt.toLowerCase().includes('résumer') || aiPrompt.toLowerCase().includes('résume')) {
                  const lines = modifiedContent.split('\n').filter(l => l.trim());
                  modifiedContent = lines.slice(0, Math.ceil(lines.length / 2)).join('\n');
                }
                
                if (aiPrompt.toLowerCase().includes('allonger') || aiPrompt.toLowerCase().includes('développer')) {
                  modifiedContent = modifiedContent + '\n\n[Section développée par l\'IA]\n\nCette section nécessite un développement supplémentaire pour clarifier les points mentionnés ci-dessus.';
                }
