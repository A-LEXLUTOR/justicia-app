...
const LoadingIndicator: React.FC = () => (
    <div className="flex items-center space-x-2">
        <span className="w-2.5 h-2.5 bg-purple-500 rounded-full animate-bounce-scale" style={{ animationDelay: '0s' }}></span>
        <span className="w-2.5 h-2.5 bg-purple-500 rounded-full animate-bounce-scale" style={{ animationDelay: '0.2s' }}></span>
        <span className="w-2.5 h-2.5 bg-purple-500 rounded-full animate-bounce-scale" style={{ animationDelay: '0.4s' }}></span>
    </div>
);
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, isLoading = false, className = '', onEditMessage, onCreateDocument }) => {
            ...
            <div className="flex-1 min-w-0">
                ...
                {isAssistant && !isLoading && (
                    <div className="flex items-center gap-2 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                            onClick={() => onCreateDocument?.(message.content)}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-green-500/10 text-green-400 hover:bg-green-500/20 border border-green-500/30 transition"
