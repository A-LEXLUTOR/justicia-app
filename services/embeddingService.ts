/**
 * Service de génération d'embeddings vectoriels avec l'API OpenAI
...
class EmbeddingService {
    ...
    constructor(apiKey: string = OPENAI_API_KEY, model: string = EMBEDDING_MODEL) {
        this.apiKey = apiKey;
        this.model = model;
    }
    ...
    async generateEmbeddingsBatch(texts: string[]): Promise<EmbeddingResult[]> {
        ...
        try {
            // Traiter par lots de 100 (limite de l'API OpenAI)
            const batchSize = 100;
